...
class _s2v_lib(object):
    ...
    def _prepare_graph(self, graph_list, is_directed=0):
        ...
        edge_pair_list = (ctypes.c_void_p * len(graph_list))()
        ...
        for i in range(len(graph_list)):
            if type(graph_list[i].edge_pairs) is ctypes.c_void_p:
                edge_pair_list[i] = graph_list[i].edge_pairs
            elif type(graph_list[i].edge_pairs) is np.ndarray:
                edge_pair_list[i] = ctypes.c_void_p(graph_list[i].edge_pairs.ctypes.data)
            else:
        ...
        self.lib.PrepareBatchGraph(self.batch_graph_handle, 
                                ...
                                ctypes.cast(edge_pair_list, ctypes.c_void_p),
                                is_directed)

        return total_num_nodes, total_num_edges

    def PrepareMeanField(self, graph_list, weights, is_directed=0):
        ...
        self.lib.PrepareMeanField(self.batch_graph_handle, ctypes.cast(idx_list, ctypes.c_void_p), ctypes.cast(val_list, ctypes.c_void_p))

        if weights is not None:
            n2n_vals = weights[:, 0].cpu()
